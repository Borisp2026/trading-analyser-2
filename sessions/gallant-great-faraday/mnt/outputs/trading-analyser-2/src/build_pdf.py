...
def build_pdf_report(results: list, portfolio: dict, output_path: str,
    ...
    if accuracy and accuracy.get("total_signals", 0) > 0:
        story.append(Paragraph("Signal Accuracy (30-Day History)", h2))
        overall = accuracy.get("overall_accuracy", 0)
        total_sig = accuracy.get("total_signals", 0)
        correct_sig = accuracy.get("correct_signals", 0)
        acc_data = [
            ["Metric", "Value"],
            ["Overall Accuracy", f"{overall:.1f}%"],
            ["Total Resolved Signals", str(total_sig)],
            ["Correct Signals", str(correct_sig)],
            ["Wrong Signals", str(total_sig - correct_sig)],
        ]
        by_stock = accuracy.get("by_stock", {})
        if by_stock:
            best = max(by_stock.items(), key=lambda x: x[1]["accuracy"], default=None)
            worst = min(by_stock.items(), key=lambda x: x[1]["accuracy"], default=None)
            if best:
                acc_data.append(["Best Stock", f"{best[0]}: {best[1]['accuracy']:.0f}% ({best[1]['correct']}/{best[1]['total']})"])
            if worst and worst[0] != (best[0] if best else None):
                acc_data.append(["Needs Review", f"{worst[0]}: {worst[1]['accuracy']:.0f}% ({worst[1]['correct']}/{worst[1]['total']})"])
        acc_table = Table(acc_data, colWidths=[(PAGE_W - 2 * MARGIN) * 0.4, (PAGE_W - 2 * MARGIN) * 0.6])
        acc_table.setStyle(TableStyle([
            ...
            ("TOPPADDING", (0, 0), (-1, -1), 5),
            ("BOTTOMPADDING", (0, 0), (-1, -1), 5),
            ("TEXTCOLOR", (1, 1), (1, 1), GREEN if overall >= 60 else ORANGE if overall >= 50 else RED),
        ]))
